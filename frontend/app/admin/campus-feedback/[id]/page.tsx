...
function submitterHubLabel(feedbackType?: string): string {
  if (feedbackType?.toLowerCase() === "compliment") return "Complimenter Hub";
  return "Complainant Hub";
}
...
export default function ComplaintDetail() {
  ...
  const fetchComplaint = async () => {
    setLoading(true);
    try {
      const data = await getApi(`/api/campus-feedback/admin/${id}`);
      setComplaint(data);
    } catch (err) {
      toast.error("Failed to load case data");
      router.push("/admin/campus-feedback");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      await putApi(`/api/campus-feedback/admin/${id}/status`, { status: newStatus });
  ...
  const handleResponse = async (e: React.FormEvent) => {
    ...
    setActionLoading(true);
    try {
       await postApi(`/api/campus-feedback/admin/${id}/responses`, {
  ...
  const handleAssign = async (staffId: string) => {
    ...
    setActionLoading(true);
    try {
       await putApi(`/api/campus-feedback/admin/${id}/assign`, { staff_id: staffId });
  ...
  const handleSuggestResponse = async () => {
    ...
    try {
      const data = await getApi(`/api/campus-feedback/admin/${id}/suggest-response`);
      if (data?.suggestion) {
  ...
  if (!complaint) return (
    <div className="flex flex-col items-center justify-center py-40 space-y-4">
      ...
      <h2 className="text-xl font-black text-primary-darker uppercase tracking-widest">Access Denied / Not Found</h2>
      <p className="text-sm text-slate-500 font-medium">You do not have permission to view this case, or it does not exist.</p>
      <Link href="/admin/campus-feedback" className="text-primary hover:underline text-sm font-bold mt-4 inline-flex items-center gap-2">
        <ArrowLeft size={16} /> Return to Feedback Hub
      </Link>
    </div>
  );
      ...
      <div className="space-y-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="flex items-center gap-8">
            <Link href="/admin/campus-feedback" className="p-4 bg-white border border-slate-200 rounded-[1.5rem] text-slate-400 hover:text-primary transition-all shadow-sm">
              <ArrowLeft size={20} />
            </Link>
            <div>
              ...
              <h2 className="text-4xl font-black text-primary-darker font-serif lowercase tracking-tighter capitalize ">Resolution &amp; Redress Hub</h2>
            </div>
          </div>
           ...
           <aside className="space-y-12">
              ...
              <div className="bg-white border border-slate-200 p-10 rounded-[3rem] shadow-sm space-y-8">
                 <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-xs font-black uppercase tracking-widest text-primary-darker font-serif">
                      {submitterHubLabel(complaint.feedback_type)}
                    </h4>
                 </div>
