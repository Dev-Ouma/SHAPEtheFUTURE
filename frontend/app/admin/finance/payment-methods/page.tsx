...
export default function PaymentMethodsAdmin() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getApi(`/finance/admin/payment-methods?page=${page}&limit=10&search=${encodeURIComponent(debouncedSearch)}`);
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to permanently delete this payment method?")) {
      try {
        await deleteApi(`/finance/admin/payment-methods/${id}`);
        fetchData();
      } catch (error) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-primary-darker uppercase tracking-tighter">Payment Methods</h1>
          <p className="text-slate-500 font-medium mt-1">Manage bank accounts, mobile money, and payment instructions.</p>
        </div>
        <Link 
          href="/admin/finance/payment-methods/create"
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-widest font-black text-slate-500">
              <tr>
                ...
                <th className="px-6 py-4">Account/Paybill No.</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium animate-pulse">Loading payment methods...</td>
                </tr>
              ) : methods.length === 0 ? (
                <tr>
                ...
                methods.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4 font-bold text-primary-darker">{m.provider_name}</td>
                    <td className="px-6 py-4 text-slate-500 font-medium">{m.account_name}</td>
                    <td className="px-6 py-4 font-mono font-bold tracking-wider">{m.account_number}</td>
                    <td className="px-6 py-4">
                      {m.is_active ? <CheckCircle className="text-emerald-500" size={18} /> : <XCircle className="text-rose-500" size={18} />}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link 
                          href={`/admin/finance/payment-methods/${m.id}`}
