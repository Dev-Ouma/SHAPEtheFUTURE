...
export default function ScholarshipsAdmin() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getApi(`/finance/admin/scholarships?page=${page}&limit=10&search=${encodeURIComponent(debouncedSearch)}`);
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to permanently delete this scholarship?")) {
      try {
        await deleteApi(`/finance/admin/scholarships/${id}`);
        fetchData();
      } catch (error) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-primary-darker uppercase tracking-tighter">Scholarships</h1>
          <p className="text-slate-500 font-medium mt-1">Manage financial aid and scholarship programmes.</p>
        </div>
        <Link 
          href="/admin/finance/scholarships/create"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium animate-pulse">Loading scholarships...</td>
                </tr>
                ...
                scholarships.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="font-bold text-primary-darker">{s.title}</div>
                      <div className="text-xs text-slate-500 line-clamp-1 max-w-xs">{s.description}</div>
                    </td>
                    <td className="px-6 py-4 font-bold text-slate-600">KES {Number(s.amount).toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-500">
                      {s.application_deadline ? new Date(s.application_deadline).toLocaleDateString() : 'No Deadline'}
                    </td>
                    <td className="px-6 py-4">
                      {s.is_active ? <CheckCircle className="text-emerald-500" size={18} /> : <XCircle className="text-rose-500" size={18} />}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link 
                          href={`/admin/finance/scholarships/${s.id}`}
