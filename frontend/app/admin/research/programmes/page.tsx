...
export default function AdminProgrammesList() {
  ...
  const fetchProgrammes = async () => {
    ...
    try {
      ...
      const response = await getApi(`/research/programmes?${params.toString()}`);
      
      setProgrammes(response.data || []);
      setTotal(response.total || 0);
      setLastPage(response.lastPage || Math.ceil((response.total || 0) / limit));
    } catch (e) {
      toast.error('Failed to load research programmes');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Are you sure you want to delete programme "${title}"?`)) return;
    try {
      await deleteApi(`/research/programmes/${id}`);
  ...
  return (
    <div className="p-10 space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-4xl font-black text-primary-darker tracking-tighter uppercase italic font-serif">Research Programmes</h1>
          <p className="text-sm font-medium text-slate-500 mt-2 uppercase tracking-widest opacity-80">Strategic Institutional Research Pillars</p>
        </div>
        <div className="flex items-center gap-4">
          <Link 
            href="/admin/research/programmes/form" 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-darker text-white uppercase tracking-[0.25em] font-black text-[10px]">
                <th className="px-10 py-6">Research Programme / Focus</th>
                <th className="px-10 py-6">Host School</th>
                <th className="px-10 py-6">Leadership</th>
                <th className="px-10 py-6">Status</th>
                <th className="px-10 py-6 text-right">Administrative Controls</th>
              </tr>
            </thead>
              ...
              {loading && programmes.length === 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="py-48 text-center bg-slate-50/50">
                    <Activity size={80} className="mx-auto text-slate-100 mb-8 opacity-50" />
                    <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-sm italic font-serif">Registry is currently empty</p>
                    <p className="text-[10px] text-slate-300 uppercase tracking-widest mt-4">Zero programmes found for current filters</p>
                  </td>
                </tr>
              ) : programmes.map(programme => (
                <tr key={programme.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                  <td className="px-10 py-8">
                     <div className="flex flex-col gap-3">
                        ...
                        <div className="text-[10px] text-slate-400 font-medium line-clamp-1 max-w-md uppercase tracking-wider">
                           {programme.summary || programme.overview?.slice(0, 100) + '...'}
                        </div>
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex items-center gap-4 text-[11px] font-black uppercase tracking-widest text-slate-600">
                        <Layers size={18} className="text-[#037b90]/20" />
                        {programme.school?.title || programme.school?.name || 'Institutional Wide'}
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                           <div className="w-8 h-8 rounded-none bg-slate-100 flex items-center justify-center text-slate-400">
                              <User size={14} />
                           </div>
                           <div className="text-[11px] font-black text-primary-darker tracking-tight uppercase">
                              {programme.lead_researcher?.full_name || 'Unassigned'}
                           </div>
                        </div>
                        <div className="text-[9px] text-slate-400 font-black font-serif italic flex items-center gap-2">
                           <Calendar size={12} />
                           {programme.start_date ? new Date(programme.start_date).getFullYear() : 'Ongoing'}
                        </div>
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <span className={`px-5 py-2 text-[9px] font-black uppercase tracking-[0.2em] rounded-none border ${
                       programme.status === 'active' ? 'bg-blue-50 text-blue-600 border-blue-100' :
                       programme.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                  ...
                  <td className="px-10 py-8 text-right">
                     <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <Link 
                           href={`/admin/research/programmes/form?id=${programme.id}`} 
                        ...
                        <button 
                           onClick={() => handleDelete(programme.id, programme.title)} 
                           className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-white hover:bg-[#ff7f50] transition-all shadow-sm"
                        >
                           <Trash2 size={18} />
                        </button>
        ...
        {!loading && total > 0 && (
          <div className="bg-white border-t border-slate-100 p-8 flex items-center justify-between">
            <div className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">
               Page <span className="text-primary-darker mx-2">{page}</span> of <span className="text-primary-darker mx-2">{lastPage}</span>
            </div>
            <div className="flex items-center gap-4">
               <button 
                 disabled={page === 1}
