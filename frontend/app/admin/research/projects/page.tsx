...
export default function AdminProjectsList() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const response = await getApi(`/research/projects?${params.toString()}`);
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      await deleteApi(`/research/projects/${id}`);
  ...
  return (
    <div className="p-10 space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-4xl font-black text-primary-darker tracking-tighter uppercase italic font-serif">Research Projects</h1>
          <p className="text-sm font-medium text-slate-500 mt-2 uppercase tracking-widest opacity-80">Institutional Research and Innovation Roadmap</p>
        </div>
        <div className="flex items-center gap-4">
          <Link 
            href="/admin/research/projects/form" 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-darker text-white uppercase tracking-[0.25em] font-black text-[10px]">
                <th className="px-10 py-6">Research Initiative / Scale</th>
                <th className="px-10 py-6">Leading Hub / School</th>
                <th className="px-10 py-6">Budget / Currency</th>
              ...
              {loading && projects.length === 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="py-48 text-center bg-slate-50/50">
                    <Briefcase size={80} className="mx-auto text-slate-100 mb-8 opacity-50" />
                    <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-sm italic font-serif">Registry is currently empty</p>
                    <p className="text-[10px] text-slate-300 uppercase tracking-widest mt-4">Zero projects found for current filters</p>
                  </td>
                </tr>
                ...
                <tr key={project.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                     ...
                     <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-4">
                           ...
                           <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                               Lead: {project.principal_investigator?.full_name || 'Unassigned'}
                           </div>
                        </div>
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex items-center gap-4 text-[11px] font-black uppercase tracking-widest text-slate-600">
                        ...
                        {project.school?.title || project.school?.name || 'Institutional Wide'}
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex flex-col gap-2">
                        <div className="text-[11px] font-black text-primary-darker tracking-tight">
                           {project.currency || 'KES'} {parseFloat(project.budget || '0').toLocaleString()}
                        </div>
                  ...
                  <td className="px-10 py-8">
                     <span className={`px-5 py-2 text-[9px] font-black uppercase tracking-[0.2em] rounded-none border ${
                       project.status === 'ongoing' ? 'bg-blue-50 text-blue-600 border-blue-100' :
                  ...
                  <td className="px-10 py-8 text-right">
                     <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <Link 
                          href={`/admin/research/projects/form?id=${project.id}`} 
