...
import { Plus, Search, Edit3, Trash2, Eye, BookOpen, User, Calendar, GraduationCap, ChevronLeft, ChevronRight, Filter, X, ChevronDown } from 'lucide-react';
...
export default function AdminPublicationsList() {
  return (
    <PermissionGate permission={['programmes.view', 'programmes.manage']}>
      <AdminPublicationsListInner />
    </PermissionGate>
  );
}

function AdminPublicationsListInner() {
  const { can: canManage } = usePermission('programmes.manage');
  ...
  // Sync search with URL after debounce
  useEffect(() => {
  ...
  const fetchPublications = async () => {
    ...
    try {
      ...
      const response = await getApi(`/research/publications/admin?${params.toString()}`);
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!canManage) {
      toast.error('You need programmes.manage permission to delete publications');
      return;
    }
    if (!confirm(`Are you sure you want to delete "${title}"?`)) return;
    try {
      await deleteApi(`/research/publications/${id}`);
  ...
  const publicationTypes = [
    ...
    { label: 'Technical Reports', value: 'technical_report' },
    { label: 'Thesis / Dissertations', value: 'thesis' },
  ];
  ...
  return (
    <div className="p-10 space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-4xl font-black text-primary-darker tracking-tighter uppercase italic font-serif">Research Repository</h1>
          <p className="text-sm font-medium text-slate-500 mt-2 uppercase tracking-widest opacity-80">Full Institutional Intellectual Property Registry</p>
        </div>
        <div className="flex items-center gap-4">
          {canManage && (
          <Link 
            href="/admin/research/publications/form" 
      ...
      <div className="bg-white border border-slate-100 flex flex-col lg:flex-row lg:items-stretch shadow-md">
        ...
        <div className="flex-grow p-4 lg:p-0 flex flex-wrap lg:flex-nowrap items-center gap-4 lg:gap-0">
          ...
          <div className="relative flex-1 min-w-[140px] lg:border-r border-slate-100 group">
            <select 
              value={searchParams.get('year') || ""}
              onChange={(e) => updateQuery({ year: e.target.value || null, page: '1' })}
              className="appearance-none w-full bg-transparent px-8 py-6 text-[10px] font-black uppercase tracking-widest outline-none focus:bg-slate-50 transition-all cursor-pointer"
            >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-darker text-white uppercase tracking-[0.25em] font-black text-[10px]">
                <th className="px-10 py-6">Intellectual Property / Title</th>
                <th className="px-10 py-6">Academic School</th>
                <th className="px-10 py-6">Type / Year</th>
                <th className="px-10 py-6">Status</th>
                <th className="px-10 py-6 text-right">Repository Actions</th>
              </tr>
            </thead>
            <tbody className={`${loading ? 'opacity-40' : ''} transition-opacity`}>
              {loading && publications.length === 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="py-48 text-center bg-slate-50/50">
                    <BookOpen size={80} className="mx-auto text-slate-100 mb-8 opacity-50" />
                    <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-sm italic font-serif">Registry is currently empty</p>
                    <p className="text-[10px] text-slate-300 uppercase tracking-widest mt-4">Zero results found for current filters</p>
                  </td>
                </tr>
              ) : publications.map(pub => (
                <tr key={pub.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                     ...
                     <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-4">
                           ...
                           <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                              {pub.staff_authors?.map((a: any) => a.full_name).join(', ') || pub.external_authors || 'No authors'}
                           </div>
                        </div>
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex items-center gap-4 text-[11px] font-black uppercase tracking-widest text-slate-600">
                        <GraduationCap size={18} className="text-[#037b90]/20" />
                        {pub.school?.name || 'Central Archives'}
                     </div>
                  </td>
                  <td className="px-10 py-8">
                     <div className="flex flex-col gap-2">
                        <div className="text-[10px] font-black text-[#037b90] uppercase tracking-widest bg-[#037b90]/5 px-3 py-1 border border-[#037b90]/10 w-fit">
                           {pub.type?.replace('_', ' ')}
                        </div>
                  ...
                  <td className="px-10 py-8">
                     <span className={`px-5 py-2 text-[9px] font-black uppercase tracking-[0.2em] rounded-none border ${
                       pub.status === 'Published' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                  ...
                  <td className="px-10 py-8 text-right">
                     <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        {canManage ? (
                          <>
                            <Link 
                              href={`/admin/research/publications/form?id=${pub.id}`} 
                              className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-white hover:bg-[#ff7f50] transition-all shadow-sm"
                            >
                              <Edit3 size={18} />
                            </Link>
                          ...
                          <span className="text-[9px] font-black uppercase tracking-widest text-slate-300 px-3">View only</span>
                        )}
                     </div>
                  </td>
                </tr>
        ...
        {!loading && total > 0 && (
          <div className="bg-white border-t border-slate-100 p-8 flex items-center justify-between">
            <div className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">
               Registry Partition <span className="text-primary-darker mx-2">{page}</span> of <span className="text-primary-darker mx-2">{lastPage}</span>
            </div>
            <div className="flex items-center gap-4">
               <button 
                 disabled={page === 1}
