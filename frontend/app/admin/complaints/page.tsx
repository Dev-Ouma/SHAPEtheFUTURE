...
export default function ComplaintsDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getApi("/complaints/admin/all");
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const data = await getApi("/complaints/admin/analytics");
      setAnalytics(data);
    } catch {
  ...
  return (
    <div className="space-y-8 pb-24">
      <LegacyAdminFallbackBanner message="This is the legacy grievance console. Use the ICT Service Desk for the unified complaints and compliments queue." />
      {/* Header */}
              ...
              >
                ...
                {tab.id !== "all" && (
                  <span className={`ml-1 rounded-full w-4 h-4 text-[8px] flex items-center justify-center ${typeFilter === tab.id ? "bg-white/20" : "bg-slate-100"}`}>
                    {complaints.filter(c => c.complaint_type === tab.id).length}
                  </span>
                )}
              </button>
            ))}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {item.complaint_type && (
                            <span className="text-[9px] font-black px-2.5 py-1 rounded-full bg-slate-50 border border-slate-200 text-slate-500 flex items-center gap-1">
                              {item.complaint_type === "Student" ? <GraduationCap size={9} /> : item.complaint_type === "Staff" ? <Briefcase size={9} /> : <Globe size={9} />}
                              {item.complaint_type}
                            </span>
                          )}
                          {item.is_escalated && (
