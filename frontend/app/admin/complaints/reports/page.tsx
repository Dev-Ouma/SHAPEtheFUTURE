...
export default function ComplaintsReports() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [analyticsData, complaintsData] = await Promise.all([
        getApi("/complaints/admin/analytics"),
        getApi("/complaints/admin/all"),
      ]);
  ...
  const byType = useMemo(() => {
    ...
    filteredComplaints.forEach(c => { const t = c.complaint_type || "External"; map[t] = (map[t] || 0) + 1; });
  ...
  return (
    <div className="space-y-8 pb-24">
      <LegacyAdminFallbackBanner message="Legacy grievance reports. Use the ICT Service Desk for unified analytics." />
      {/* Header */}
